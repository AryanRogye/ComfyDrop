...
- Apple’s minimum acceptable tap area for interactions on iOS is 44x44. Ensure this is strictly enforced there. On macOS, do not inflate every control to touch sizes, but do keep controls comfortably clickable and visually balanced.
- For macOS layouts, review the design at multiple window widths rather than assuming a single fixed canvas.
...
- Avoid UIKit/AppKit colors (`UIColor` or `NSColor`) in SwiftUI code; use SwiftUI `Color` or asset catalog colors unless bridging is required.
