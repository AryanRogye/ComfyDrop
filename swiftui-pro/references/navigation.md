...
- Use `NavigationStack` or `NavigationSplitView` as appropriate; flag all use of the deprecated `NavigationView`. macOS apps with persistent sidebars usually want `NavigationSplitView`.
...
- On macOS, app-wide actions such as preferences, import/export, and workspace management often belong in commands, menus, or toolbars rather than inside pushed navigation content.
...
- On macOS, prefer popovers, inspectors, or secondary panels for lightweight supporting UI; reserve sheets for clearly modal tasks.
